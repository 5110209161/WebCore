...
import { NavigationItem } from 'src/app/domain/navigation-item';
...
export class LayoutMenuComponent implements OnInit {
  ...
  routes: NavigationItem[] = [
      ...
      {
          label: 'Table', value: 'table', isSubItem: true,
          items: [
  ...
  /** Callback for menu search box to invoke searching */
  filterGroupedRoute(event): void {
    let query = event.query;
    let filteredGroups = [];

    this.routes.forEach(optgroup => {
  ...
  /** select event of menu search box  */
  onSelect(event): void {
  ...
  scrollToSelectedRoute() {
    let routeEl = DomHandler.findSingle(this.elementRef.nativeElement, '.router-link-exact-active');
  ...
  /** check if the sub menu item is active */
